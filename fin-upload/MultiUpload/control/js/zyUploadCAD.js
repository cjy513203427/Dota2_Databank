	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "http://106.14.213.208:8889/rest/cadserver/uploadServerCAD",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(dragDrop){
	            	...
	            	html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" multiple="multiple">';
	            	html += '					</div>';
	            	html += '				</div>';
		            ...
		            html += '					<input type="submit" value="开始上传" class="upload_btn">';
		            html += '				</div>';
		            html += '			</div>';
					html += '			<div id="preview" class="upload_preview"></div>';
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<input id="fileSubmit" class="upload_submit_btn" type="submit" value="开始上传">';
					html += '		</div>';
		            ...
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="cadPath" '+multiple+'>';
                    html += '					<input id="cadPathName" type="text" size="30" class="cadPathName" name="cadPathName">';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">上传成功</p>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
                        var str = document.getElementById('fileImage').value;
                        var index = str .lastIndexOf("\\");
                        //alert(str.substring(index + 1, str.length));
                        $('#cadPathName').val(str.substring(index + 1, str.length));
						alert($('#cadPathName').val());
